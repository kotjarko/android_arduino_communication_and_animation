...
public class ArduinoCommunicatorService extends Service {
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        if (!intent.getBooleanExtra(UsbManager.EXTRA_PERMISSION_GRANTED, false)) {
            ...
            if(DEBUG)Toast.makeText(getBaseContext(), getString(R.string.permission_denied), Toast.LENGTH_LONG).show();
            stopSelf();
            return Service.START_REDELIVER_INTENT;
        }
        ...
        if(DEBUG)Toast.makeText(getBaseContext(), getString(R.string.receiving), Toast.LENGTH_SHORT).show();
        startReceiverThread();
        startSenderThread();
    ...
    private boolean initDevice() {
        ...
        if (mUsbConnection == null) {
            ...
            if(DEBUG)Toast.makeText(getBaseContext(), getString(R.string.opening_device_failed), Toast.LENGTH_LONG).show();
            return false;
        }
        ...
        if (!mUsbConnection.claimInterface(usbInterface, true)) {
            ...
            if(DEBUG)Toast.makeText(getBaseContext(), getString(R.string.claimning_interface_failed), Toast.LENGTH_LONG).show();
            mUsbConnection.close();
            return false;
        }
        ...
        // Set line encoding.
        mUsbConnection.controlTransfer(0x21, 0x20, 0, 0, getLineEncoding(115200), 7, 0); //9600
        ...
        if (mInUsbEndpoint == null) {
            ...
            if(DEBUG)Toast.makeText(getBaseContext(), getString(R.string.no_in_endpoint_found), Toast.LENGTH_LONG).show();
            mUsbConnection.close();
            return false;
        }
        ...
        if (mOutUsbEndpoint == null) {
            ...
            if(DEBUG)Toast.makeText(getBaseContext(), getString(R.string.no_out_endpoint_found), Toast.LENGTH_LONG).show();
            mUsbConnection.close();
            return false;
        }
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (SEND_DATA_INTENT.equals(action)) {
                ...
                if (dataToSend == null) {
                    ...
                    if(DEBUG)Toast.makeText(context, text, Toast.LENGTH_LONG).show();
                    return;
                }

                mSenderThread.mHandler.obtainMessage(10, dataToSend).sendToTarget();
            } else if (UsbManager.ACTION_USB_DEVICE_DETACHED.equals(action)) {
                if(DEBUG)Toast.makeText(context, getString(R.string.device_detaches), Toast.LENGTH_LONG).show();
        ...
        new Thread("arduino_receiver") {
            public void run() {
                ...
                while(mUsbDevice != null ) {
                    ...
                    final int len = mUsbConnection.bulkTransfer(mInUsbEndpoint, inBuffer, 1, 0);//inBuffer.length, 0);
                    if (len > 0) {
